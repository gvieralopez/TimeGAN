...
def discriminative_score_metrics(ori_data, generated_data):
    ...
    X_hat = tf.compat.v1.placeholder(
        tf.float32, [None, max_seq_len, dim], name="myinput_x_hat"
    )
    ...
    def discriminator(x, t):
        ...
        with tf.compat.v1.variable_scope(
            ...
            d_cell = tf.keras.layers.GRUCell(
                hidden_dim, activation="tanh", name="d_cell"
            )
            d_rnn = tf.keras.layers.RNN(
                d_cell, return_sequences=True, return_state=True
            )
            d_outputs, d_last_states = d_rnn(x)
            y_hat_logit = tf.keras.layers.Dense(1, activation=None)(d_last_states)
            y_hat = tf.nn.sigmoid(y_hat_logit)
            d_vars = [
                v for v in tf.compat.v1.global_variables() if v.name.startswith(vs.name)
            ]
